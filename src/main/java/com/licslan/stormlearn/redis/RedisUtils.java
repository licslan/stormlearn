package com.licslan.stormlearn.redis;
...
public class RedisUtils {
    ...
    private static class RedisService{
        private static RedisUtils redis = new RedisUtils("192.168.126.128",6380);
    }


    /** 获取JRedis设置为公开的  其他方法改成private */
    public static RedisUtils getJRedis(){
     ...
     * @param prot 端口
     */
    private RedisUtils(String ip, int prot) {
        if (pool == null) {
     ...
     * @param prot 端口
     */
    private RedisUtils(JedisPoolConfig config ,String ip, int prot){
        if (pool == null) {
     ...
     * @param timeout 超时时间
     */
    private RedisUtils(JedisPoolConfig config ,String ip, int prot ,int timeout){
        if (pool == null) {
     ...
     * @param pool 连接池对象
     */
    private RedisUtils(JedisPool pool){
        if (this.pool == null) {
            this.pool = pool;
        }
    }
    ...
    public static void main(String[] args) {
        RedisUtils redisutil=new RedisUtils("localhost",6379);
